...
<asp:Content ID="Content2" ContentPlaceHolderID="Body" runat="server">
    ...
    <section>
        <h3>高级任务管理</h3>
        <div>在这里你可以管理高级任务（即非单文件输出的任务，如无文件输出或多文件输出），以下显示了当前正在进行的 <%=TaskCount %> 项任务，选中了 <span id="selected-count">0</span> 项任务。如果你要处理其他任务（即单文件输出任务），只需点击/删除输出文件即可。</div>
    ...
    <section class="button-set">
        <button type="button" onclick="doSelectAll();">全选</button>
        <button type="button" onclick="invertSelection();">反选</button>
        <%  if (Request.GetUser().OperateTasks)
            { %>
        <button runat="server" OnServerClick="Delete" onclick="if (deleteConfirm())">删除任务及其相关数据</button>
        <%  } %>
    </section>
    <section>
        <table id="task-list" class="hovered bordered table">
            <thead>
                <tr>
                    <th>类型</th>
                    <th>添加时间</th>
